...
export default function ManagerApprovalsPage() {
  ...
  const fetchAbsences = async () => {
    try {
      ...
      console.log('📋 Set absences:', data.absences?.length || 0);
    } catch (err: any) {
  ...
  const handleApproval = async (absenceId: string, status: 'approved' | 'rejected') => {
    try {
      ...
      console.log(`🔄 ${status === 'approved' ? 'Approving' : 'Rejecting'} absence ${absenceId}...`);
      ...
      console.log(`✅ Absence ${status === 'approved' ? 'approved' : 'rejected'} successfully`);
